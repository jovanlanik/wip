...
wip_ply_t *wip_readModel(wip_ply_t *p, char *file) {
	...
	msh_ply_desc_t wip_modelFormatVertex = {
		.element_name = "vertex",
	...
	wip_modelFormatVertex.data = &p->vertex;
	wip_modelFormatVertex.data_count = &p->vertex_c;

	wip_modelFormatColor.data = &p->color;
	wip_modelFormatColor.data_count = &p->vertex_c;

	wip_modelFormatNormal.data = &p->normal;
	wip_modelFormatNormal.data_count = &p->vertex_c;

	wip_modelFormatFace.data = &p->index;
	wip_modelFormatFace.data_count = &p->index_c;

	int r = 0;
	msh_ply_t *ply_file = msh_ply_open(file, "r");
	if(!ply_file) wip_log(WIP_FATAL, "%s: Couldn't open model %s", __func__, file);
	r = msh_ply_add_descriptor(ply_file, &wip_modelFormatVertex);
