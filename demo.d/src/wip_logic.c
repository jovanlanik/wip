...
void *wip_logicThread(void *arg) {
	...
	while(!wip_globalWindow.close) {
		...
		//quat_rotate(object[0].rotation, 25*startTime, (float[]){0, 1, 0});
		quat rot, cop;
		quat_dup(cop, object[0].rotation);
		quat_rotate(rot, TO_RAD(0.15f), (float[]){0, 1, 0});
		quat_mul(object[0].rotation, cop, rot);
		quat_norm(object[0].rotation, object[0].rotation);
